...
from qiskit.test.mock.backends.brisbane import FakeBrisbane
...
transpiled_qc = transpile(qc, backend=backend, optimization_level=3, seed_transpiler=11)

# 4. Print results
print("Qiskit optimization_level=3 results (with seed=11) for FakeBrisbane:")
print(f"Depth: {transpiled_qc.depth()}")
ops = transpiled_qc.count_ops()
cnot_count = ops.get('cx', 0) + ops.get('ecr', 0)
print(f"2-qubit gates: {cnot_count}")
